...
;
;       WEIGHT_OUT = set this optional keyword to a named variable to pass
;                    back the weight vector.  We take the liberty of
;                    forcing the weight to zero for any lambda-squared
;                    value whose corresponding Q or U image is completely
;                    masked out with IEEE NaN.  Later, when running
;                    RMCLEAN, you will want to pass this weight vector into
;                    that routine... the RM Spread Function depends on the
;                    weight vector and lambda-squared values should not be
;                    included in the calculation of the RMSF if the
;                    corresponding data are masked out.  We suggest
;                    calculating the RMSF explicitly and passing it into
;                    RMCLEAN if you're worried about masking since
;                    it will force you, the user, to consider what
;                    you're doing.
;
; OUTPUTS:
...
;       Fixed error in forcing zero weights for lambda-squared
;       channels that have entire image plane masked out. TR 24 May 2013
;       Allow user to pass back WEIGHT_OUT as keyword since we set any
;       input weight values to zero if Q or U data for corresponding
;       lambda-squared is masked out with IEEE NaN... weight vector needs
;       to be passed to RMCLEAN to correctly calculate RMSF. TR 04 Jun 2013
;-
...
case szq[0] of
   ...
   2 : begin
      ...
      blank = where(total(blank_mask,2) eq nx*ny, nblank)
   end
   ; GIVE US A 3D INPUT AND WE GIVE BACK A [NX,NY,NPHI] ARRAY...   
   3 : begin
      nx = szq[2]
      ny = szq[3]
      blank = where(total(total(blank_mask,2),2) eq nx*ny, nblank)
   end
...
; SET WEIGHT TO ZERO FOR LAMBDA-SQUARED CHANNELS THAT HAVE ENTIRE IMAGE
; PLANE BLANKED...
if (nblank gt 0) then weight[blank] = 0.0d

; IF THE USER WANTS THE WEIGHT_OUT, THEN PASS IT BACK...
if arg_present(WEIGHT_OUT) then weight_out = weight
